...
export function AuthProvider({ children }) {
    ...
    useEffect(() => {
        ...
        fetch(`${ENV.API_URL}/auth/me`, { headers: { Authorization: `Bearer ${stored}` } })
    ...
    const logout = useCallback(async () => {
        ...
        if (t) {
            fetch(`${ENV.API_URL}/auth/logout`, { method: 'POST', headers: { Authorization: `Bearer ${t}` } }).catch(() => { })
        }
        clearSession()
    }, [])
    ...
    const refreshQuota = useCallback(async () => {
        ...
        if (!t) return
        try {
            const res = await fetch(`${ENV.API_URL}/auth/me`, { headers: { Authorization: `Bearer ${t}` } })
            if (res.ok) {
                const data = await res.json()
                updateUser({
                    request_quota: data.request_quota ?? 20,
